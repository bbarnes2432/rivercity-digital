 ...
 *
 * Named shader-background.tsx rather than the upstream kk.tsx: this is the
 * hero background of the page paid search points at, and a file called kk
 * tells the next person nothing.
 *
 * The upstream component is well built — it already pauses offscreen and on
 * hidden tabs, caps DPR at 2, caps the pixel budget at 2M, uses pointer events
 * rather than mouse events, and releases the WebGL context on unmount. Four
 * things were changed:
 *
 *  1. A CSS GRADIENT FALLBACK. Upstream bails with `if (!gl) return`, and
 *     never checks COMPILE_STATUS or LINK_STATUS, so any driver that refuses
 *     the shader leaves a blank canvas. That was survivable when a photograph
 *     sat behind it. It is not survivable now that the shader IS the
 *     background: the hero would render as a void. The gradient is painted on
 *     the wrapper, so it shows through whenever WebGL is missing or fails.
 *
 *  2. prefers-reduced-motion. Upstream animates unconditionally. Here the
 *     drift and time advance are frozen, leaving a still gradient.
 *
 *  3. BRAND PALETTE. The stock preset ends on #EAF9FF — near-white — and this
 *     hero renders white text over it. See PALETTE below.
 *
 *  4. The scroll handler is rAF-throttled. Upstream calls
 *     getBoundingClientRect() synchronously on every scroll event in the
 *     capture phase, which forces layout on each one; on mobile that is a
 *     measurable source of scroll jank.
 */
...
/* PALETTE — retuned from the stock preset.
 *
 * The preset ran dark-navy → blue → bright-cyan → #EAF9FF, and that last stop
 * is near-white. This hero renders white display type, a white lede and a
 * white-bordered phone button directly over it, so wherever a near-white blob
 * drifted under the text the copy would vanish. That is a WCAG failure and,
 * more to the point, a conversion failure on the page paid search lands on.
 *
 * The preset's shape is kept — dark base, mid blue, bright accent — but the
 * range is pulled into the brand and topped out at brand teal instead of
 * white, so the brightest possible pixel still holds contrast under the
 * hero's existing navy scrim. Measured after the change, not assumed.
 */
const PALETTE: [number, number, number][] = [
  [0.0392, 0.0784, 0.1294], // #0A1421 near-black navy
  [0.0627, 0.1137, 0.1922], // #101D31 brand deep navy
  [0.1059, 0.3686, 0.5490], // #1B5E8C river blue
  [0.2980, 0.6471, 0.6784], // #4CA5AD BRAND teal — brightest stop
  [0.2980, 0.6471, 0.6784],
  [0.2980, 0.6471, 0.6784],
  [0.2980, 0.6471, 0.6784],
  [0.2980, 0.6471, 0.6784],
]

/* Painted on the wrapper, behind the canvas. This is what a visitor sees if
 * WebGL is unavailable, blocked, out of contexts, or the shader fails to
 * compile — instead of an empty hero. Mirrors the palette's dark end. */
const FALLBACK_GRADIENT =
  "radial-gradient(120% 100% at 72% 18%, #1B5E8C 0%, #14314C 38%, #101D31 68%, #0A1421 100%)"
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // No context: the wrapper's gradient is already painted, so leaving now
    // shows that rather than nothing.
    if (!gl) return
