...
import { ArrowDown, ArrowUpRight, Check, Code2, FileText, Globe, LayoutTemplate, Search, ShieldCheck, ShoppingBag, Smartphone, Workflow } from "lucide-react";
...
export function BuildCapabilities() {
  return <section className="wd-section wd-deliverables rcd-light" id="included" aria-labelledby="included-heading"><div className="wd-container">
    <div className="wd-section-heading"><div><p className="wd-eyebrow">What you get / Design through launch</p><h2 id="included-heading">The design is custom.<br />The rest of the build is, too.</h2></div><p>We plan the content, design the pages, write the code, and prepare the site for everyday use.</p></div>
    <div className="wd-deliverable-grid">
      <article className="wd-design-deliverable"><div className="wd-design-photo"><Image src="/assets/card-website-design.webp" alt="Desktop displays showing two different page layout compositions" width={1280} height={720} sizes="(max-width: 760px) 90vw, 48vw" /><span>01 / CONTENT & DESIGN</span></div><div className="wd-deliverable-copy"><h3>Pages built around what your customers need to know.</h3><p>Your services, experience, reviews, and project examples determine the layout. We plan desktop and mobile together, then show you the design before development.</p><ul><li><Check size={15}/> Content and navigation planning</li><li><Check size={15}/> Brand-specific layouts and imagery</li><li><Check size={15}/> Design review and agreed revisions</li></ul></div></article>
      <div className="wd-technical-deliverables">
        <article className="wd-search-deliverable"><div className="wd-deliverable-heading"><Search size={23}/><span className="wd-eyebrow">02 / Search foundations</span></div><div className="wd-page-map" aria-hidden="true"><span><Globe size={14}/> Your business</span><ArrowDown size={18}/><div><span>Services</span><span>Locations</span><span>Contact</span></div></div><h3>Help search engines understand your business.</h3><p>Clear service pages, descriptive titles, internal links, sitemaps, and relevant structured data. Useful content and consistent business details also give AI search systems better information to work with.</p><span className="wd-deliverable-footnote">Technical setup is included. Ongoing SEO and content campaigns are scoped separately.</span></article>
        <article className="wd-launch-deliverable"><div className="wd-deliverable-heading"><ShieldCheck size={23}/><span className="wd-eyebrow">03 / Launch checks & handover</span></div><h3>Check the details customers rely on.</h3><div className="wd-check-matrix">{["Mobile layouts", "Forms & phone links", "Keyboard & contrast", "Image performance", "Analytics setup", "Redirects & domain"].map(item=><span key={item}><Check size={14}/>{item}</span>)}</div><p>You receive site access and a handover. We explain hosting, updates, and support so you know what happens after launch.</p></article>
      </div>
    </div>
    <div className="wd-project-types"><div><p className="wd-eyebrow">The projects we take on</p><h3>One landing page or a complete business platform.</h3></div><div className="wd-project-type-grid">{PROJECT_TYPES.map(({Icon,title,text})=><article key={title}><Icon size={22}/><h4>{title}</h4><p>{text}</p></article>)}</div></div>
  </div></section>;
}
...
export function BuildProcess() {
  return <section className="wd-section wd-photo-process" id="process" aria-labelledby="process-heading"><div className="wd-container">
    <div className="wd-section-heading"><div><p className="wd-eyebrow">How the project works</p><h2 id="process-heading">You see and approve<br />the work as it develops.</h2></div><BookCallLink context="website-design-process" className="wd-text-link">Book a 30-minute project call <ArrowUpRight size={17}/></BookCallLink></div>
    <div className="wd-process-cards">{PROCESS.map(step=><article key={step.n}><div className="wd-process-photo"><Image src={step.image} alt={step.alt} width={1024} height={1024} sizes="(max-width: 760px) 90vw, 31vw"/><span>{step.n}</span></div><div className="wd-process-card-copy"><h3>{step.title}</h3><p>{step.description}</p><ul>{step.points.map(point=><li key={point}><Check size={14}/>{point}</li>)}</ul><div className="wd-process-deliverable"><FileText size={16}/><span>{step.deliverable}</span></div></div></article>)}</div>
    <div className="wd-process-scope"><span className="wd-eyebrow">Before you commit</span><p>Your proposal sets out the pages, features, revisions, timeline, and price. Hosting, third-party services, and ongoing support are explained separately.</p><a href="#start">Discuss the scope <ArrowUpRight size={17}/></a></div>
  </div></section>;
}
